...
- 🛠 **Стабильность**: навигация и нижний бар пересобраны без вложенных
  `safeAreaInset` — единый стеклянный нижний блок (Док + таб-бар), меньше
  шансов на краш на iOS 26.
- 🧊 **Один стеклянный стиль везде**: выключенное состояние всех кнопок
  (таб-бар, MTProto/SOCKS5, статус-чипы) теперь тоже настоящее
  `glassEffect(.regular)`; активно — градиентная капсула с глянцем.
  Кнопки растягиваются только по своей группе (max 340 pt) — не разъезжаются.
- 📐 **Адаптивная сетка по ширине**: 1 колонка (<240 pt) / 2 (iPhone до 620)
  / 3 (iPad), контент ограничен 680 pt и отцентрован, отступы растут на
  широких экранах.
- 📶 **Пинг стал надёжнее**: таймауты рукопожатий подняты до 4–6 c, пачки
  проверок снижены до 24 (меньше параллельных коннектов = меньше ложных
  таймаутов на слабом канале), «мёртвые» прокси перепроверяются каждые 8 c
  (если сервер ожил — он снова появится зелёным).

## Что нового в 1.4.7 (iOS 1.0.4)

- 🧊 **Весь UI в одном стиле** — навигация собрана из тех же стеклянных
  капсул, что вкладки «Прокси»/«Локальный»: отдельный плавающий таб-бар,
  переключатель MTProto/SOCKS5 — две отдельные кнопки (синий/фиолетовый акцент),
  все элементы Liquid Glass.
- 🎨 **Серый градиент фона** вместо чёрного — графитовые тона, палитра
  поднята до светло-серых, карточки читаются лучше.
- 📐 **По-настоящему адаптивная сетка**: колонки не растягиваются
  (минимум 170, максимум 300 pt), контент ограничен 660 pt и центрируется —
  одинаково аккуратно на SE, Max и iPad.
- 📶 **Пинг теперь виден**: добавлен тир «сервер жив» (TCP-коннект ок, но
  MTProto-рукопожатие не прошло) — такие прокси показывают мс и серый бейдж
  вместо «мёртвых»; живые считаются честно только после ResPQ.
- 🎯 **Пинг реле в «Локальном»**: карточка туннеля показывает живую задержку
  до выбранного сервера (зелёный/красный), а при WebSocket-фолбэке — бейдж WS.
- ✍️ Кнопка-ссылка на автора теперь ведёт на `@yetilov`.
